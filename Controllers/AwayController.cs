...
namespace CooperativeOffice.Controllers
{
    public class AwayController : Controller
    {
        ...
        public ActionResult Show(string code, string euid, string returnurl, int key = 0)
        {
            ...
            var model = new DBopreator().GetAwayById(code);
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult GetAwayById(string id)
        {
            return Json(new DBopreator().GetAwayById(id), JsonRequestBehavior.AllowGet);
        }
        /// <summary>
        /// 获取草稿
        /// </summary>
        ...
        {
            return Json(new DBopreator().GetAwayDraft(orgeuid, euid), JsonRequestBehavior.AllowGet);
        }
        /// <summary>
        /// 保存审批操作
        /// </summary>
        ...
        /// <returns></returns>
        public JsonResult Save(AwayModels param)
        {
            return Json(new DBopreator().SaveAway(param));
        }
        /// <summary>
        /// 保存草稿
        /// </summary>
        /// <param name="param"></param>
        /// <returns></returns>
        public JsonResult SaveDraft(AwayModels param)
        {
            return Json(new DBopreator().SaveAwayDraft(param));
        }
        /// <summary>
        /// 发送催办通知
        /// </summary>
