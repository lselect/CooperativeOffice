...
if (callClientProxy.browserType === null || callClientProxy.browserType === "" || typeof callClientProxy.browserType === "undefined") {
    callClientProxy.browserType = urlrequest("TermType");
}
if (callClientProxy.browserType === null || callClientProxy.browserType === "" || typeof callClientProxy.browserType === "undefined") {
    //自动检测浏览器类型
    if (browser.versions.ios || browser.versions.iPhone || browser.versions.iPad) {
...
callClientProxy.browserCall = function (called, name, caller, isRecord, url, callCallback, reportCallback) {
    ...
    */
    ...
    if (this.browserType == 1) { //ios
        ...
        var url = 'UCall://call?para={"name":"{0}","called":"{1}","caller":"{2}","isRecord":"{3}","type":"1","url":"{4}","callback":"{5}","report":"{6}","hide":1}'.format(name, called, caller, isRecord, url, callCallback, reportCallback);
...
callClientProxy.loadModule = function (name) {
    ...
    if (this.browserType == 1) {//ios
        ...
        if (drhyReg.test(p)) {
            window.location.href = 'UCall://Meeting?para={"callback":""," members":[],"url":""}';
...
//启动连续地上报位置服务
callClientProxy.StartPosReport = function (mode, worktimejson, intervalTime, intervalDistance, urlReport,callback) {
    if (this.browserType == 0 || this.browserType == "") { //android
        CallClient.StartPosReport(mode, worktimejson, intervalTime, intervalDistance, urlReport);
    }
    if (this.browserType == 1) { //ios
        var url = 'UCall://StartPosReport?para={0}';
        var worktime = "";
        if (worktimejson) {
            var worktime = JSON.parse(worktimejson);
        }
        var param = {
            callback: callback,
            mode: mode,
...
//停止连续上报位置的服务
callClientProxy.StopPosReport = function (callback) {
    if (this.browserType == 0 || this.browserType == "") { //android
        CallClient.StopPosReport();
    }
    if (this.browserType == 1) { //ios
        var url = 'UCall://StopPosReport?para={"callback":"{0}"}';
        url = url.format(callback);
        window.location.href = url;
    }
    if (this.browserType == 2) { //android
...
//获取APP存储的上报位置的设置信息
callClientProxy.getLocationSetting = function (callback) {
    if (this.browserType == 0 || this.browserType == "") { //android
        CallClient.getLoactionJson();
    }
    if (this.browserType == 1) { //ios
        var url = 'UCall://getLocationSetting?para={"callback":"{0}"}';
        url = url.format(callback);
        window.location.href = url;
    }
    if (this.browserType == 2) { //android
...
callClientProxy.vibrate = function () {
    if (this.browserType == 0 || this.browserType == "") { //android
        var url = 'UCall://vibrate';
        window.location.href = url;
    }
    if (this.browserType == 1) { //ios
        var url = 'UCall://vibrate';
